...
class LDAPService:
    ...
    def search_users(self, search_base, user_dns, filter_disabled=False):
        if not user_dns:
            return []
...
class LDAPApplication:
    ...
    def run(self, search_base, group_patterns):
        ...
        users = self.ldap_service.search_users(search_base, user_dns)
        if users:
            self.ldap_service.print_entries(users)
        else:
            logger.info("No user entries found.")

        logger.info("Fetching disabled users...")
        disabled_users = self.ldap_service.search_users(search_base, user_dns, filter_disabled=True)
        if disabled_users:
            pass  # process the disabled users
        else:
